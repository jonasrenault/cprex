...
from cprex.rel.rel_pipe import make_relation_extractor, score_relations  # noqa: F401


def evaluate_model(trained_pipeline: Path, test_data: Path, print_details: bool):
    ...
    for gold in docs:
        ...
        if print_details:
            ...
            for value, rel_dict in pred._.rel.items():
                ...
                if gold_labels:
                    print(
                        f" pair: {value} --> gold labels: {gold_labels} --> "
                        f"predicted values: {rel_dict}"
                    )
            print()
