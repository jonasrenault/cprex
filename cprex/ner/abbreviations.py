"""
Abbreviations (https://github.com/allenai/scispacy?tab=readme-ov-file#abbreviationdetector)
copied from https://github.com/allenai/scispacy/blob/main/scispacy/abbreviation.py
...
def find_abbreviation(
    ...
    The algorithm works by enumerating the characters in the short form of the
    abbreviation, checking that they can be matched against characters in a
    candidate text for the long form in order, as well as requiring that the
    first letter of the abbreviated form matches the _beginning_ letter of a word.
    ...
    while short_index >= 0:
        ...
        while (
            ...
            or
            # .... or if we are checking the first character of the abbreviation,
            # we enforce to be the _starting_ character of a span.
...
class AbbreviationDetector:
    ...
    The abbreviations attribute is a `List[Span]` where each Span has the
    `Span._.long_form` attribute set to the long form definition of the abbreviation.
    ...
    make_serializable: `bool`, a required argument for whether we want to use the
    serializable or non serializable version.
    """
    ...
    def find_matches_for(
        ...
        for long_candidate, short_candidate in filtered:
            ...
            # the global matcher. So it's likely that things will work out ok most of
            # the time.
    ...
    def make_short_form_serializable(self, abbreviation: Span):
        """
        Converts the abbreviations into a short form that is serializable to
